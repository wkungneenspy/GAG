...
//        FacesContext.getCurrentInstance().addMessage(null, msg);
    ...
    public void serviceDispoAuNoeud() {
        ...
        if (selectedService != null) {
            try {
System.out.println("selectedService " + selectedService.getName());
    ...
    public String definirModeLancement() {
        try {
            if (!FileUtils.readFileToString(new File(gagconfigFile.getMutexFile()+"backendRun.txt")).equals("yes")) {
                launchBackend("");
System.out.println("Willy -> MODEEEEE "+mode+"     "+gagconfigFile.getMutexFile()+"toto.txt");
                FileUtils.writeStringToFile(new File(gagconfigFile.getMutexFile()+"toto.txt"), mode);
                FileUtils.writeStringToFile(new File(gagconfigFile.getMutexFile()+"besoin.txt"), "");
...
System.out.println("Willy 1 "+gagconfigFile.getMutexFile()+"launchBackend.bat");            
            //willy Process p = Runtime.getRuntime().exec("cmd /c "+gagconfigFile.getMutexFile()+"launchBackend.bat");
            
            new Thread() {
                @Override public void run() {
                    try {
                       Runtime.getRuntime().exec("cmd /c "+gagconfigFile.getMutexFile()+"launchBackend.bat");
                    } catch (IOException ex) {
                        Logger.getLogger(DataListView.class.getName()).log(Level.SEVERE, null, ex);
                        ex.printStackTrace();
                    }
...
System.out.println("Willy 2");
    ...
    public static void main(String[] ag) {
        try {
            ...
            while ( (ligne = br.readLine()) != null){
                    result = true;
                    System.out.println("ligneligneligne"+ligne);
                    ligne = ligne.trim().replace(" ", "###");
                    break;
            }
            
            if(result)
                Runtime.getRuntime().exec("cmd /c taskkill /F /PID "+ligne.trim().split("###")[0]);
