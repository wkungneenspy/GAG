...
public class BasicView implements Serializable {
    ...
    private List<packageService.Service> availableServicesNewCase;

    private packageService.Service selectedAvailableServiceNewCase;
    ...
    private boolean isServiceEntrant;
    ...
    List<packageService.Item> listInputItem = new ArrayList<packageService.Item>();
...
System.out.println("instanceInProgressPath ---> "+instanceInProgressPath);     
...
System.out.println("--->>>> "+gagconfigFile.getInstanceFolder());          
            ...
            try {
            //si serviceEntrant.txt contient un service alors notifier l'interface
            String serviceEntant = FileUtils.readFileToString(new File(gagconfigFile.getMutexFile()+"serviceEntrant.txt"));
            isServiceEntrant = false;
            if(serviceEntant.length() > 0){
                isServiceEntrant = true;
                System.out.println(serviceEntant+" |||||||||||| "+isServiceEntrant);
                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "Warn", "Your are received a new case, Please click on newCase button in order to follow case"));
            }
        } catch (IOException ex) {
            Logger.getLogger(BasicView.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public void newCaseTreatment(){
        try {
            System.out.println("=====> "+selectedAvailableServiceNewCase.getName());
            
            if(availableServicesNewCase.size()>1){
                    //traitement du choix
                    String numChoisie = selectedAvailableServiceNewCase.getName().split("::")[0].split("S")[1];
                    System.out.println("=====> Num Service choisi "+numChoisie);
                    
                    FileUtils.writeStringToFile(new File(gagconfigFile.getMutexFile() + "toto.txt"), numChoisie);
                    FileUtils.writeStringToFile(new File(gagconfigFile.getMutexFile() + "besoin.txt"), "");
               
            }
            
            isServiceEntrant = false;
            FileUtils.writeStringToFile(new File(gagconfigFile.getMutexFile()+"serviceEntrant.txt"), "");
        } catch (IOException ex) {
            Logger.getLogger(BasicView.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public void newCase(){
        try {
            String paramRecu = FileUtils.readFileToString(new File(gagconfigFile.getMutexFile()+"serviceEntrant.txt"));
            String serv = paramRecu.split("###")[0];
            
            //Recupere les input
            listInputItem = new ArrayList<packageService.Item>();
            String input = paramRecu.split("###input")[1];
            if(input.trim().contains(":::")){
                String[] listinput = input.trim().split(";;;");
                for(int j=0;j<listinput.length;j++){
                    if(!listinput[j].trim().isEmpty()){
                        packageService.Item item  = new packageService.Item();
                        item.setName(listinput[j].split(":::")[0]);
                        item.setContent(listinput[j].split(":::")[1]);
                        
                        listInputItem.add(item);
                    }
                }
            }
            //Fin du recupere les input
            
            
            if(serv.length()>0){
               availableServicesNewCase = findMatchingService(serv, gagconfigFile.getSchemaExecution());
               
                System.out.println("availableServicesNewCase size "+availableServicesNewCase.size());
                List<packageService.Service> services = new ArrayList<packageService.Service>();
                DataListView dlv = new DataListView();
                int i=0;
                for(packageService.Service service: availableServicesNewCase){
                ...
                availableServicesNewCase = services;
            }
                    
            
                    //1. lecture du fichier serviceEntrant.txt....
                    
                    //2. Verifie s'il y a plusieurs services
                    
                    //3. s'il y a plusieurs, alors boite de dialogue du choix
                    
                    //4. vider le fichier serviceEntrant.txt
                    
                    //5. sinon refresh la fenêtre
        } catch (IOException ex) {
            Logger.getLogger(BasicView.class.getName()).log(Level.SEVERE, null, ex);
        } catch (JAXBException ex) {
            Logger.getLogger(BasicView.class.getName()).log(Level.SEVERE, null, ex);
...
//////        }
    ...
    public List<packageService.Item> getListInputItem() {
        return listInputItem;
    }

    public void setListInputItem(List<packageService.Item> listInputItem) {
        this.listInputItem = listInputItem;
    }
    ...
    public boolean isIsServiceEntrant() {
        return isServiceEntrant;
    }

    public void setIsServiceEntrant(boolean isServiceEntrant) {
        this.isServiceEntrant = isServiceEntrant;
    }

    public List<packageService.Service> getAvailableServicesNewCase() {
        return availableServicesNewCase;
    }

    public void setAvailableServicesNewCase(List<packageService.Service> availableServicesNewCase) {
        this.availableServicesNewCase = availableServicesNewCase;
    }

    public packageService.Service getSelectedAvailableServiceNewCase() {
        return selectedAvailableServiceNewCase;
    }

    public void setSelectedAvailableServiceNewCase(packageService.Service selectedAvailableServiceNewCase) {
        this.selectedAvailableServiceNewCase = selectedAvailableServiceNewCase;
    }
}
