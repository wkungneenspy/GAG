...
public class ManipXML {
	...
	public void javatoXml(Doc expr,String cheminFichier,Artefact newArt) throws JAXBException, FileNotFoundException{
		...
		Marshaller marshaller = jc.createMarshaller();
		marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, new Boolean(true));
		marshaller.marshal(res,new FileOutputStream(cheminFichier));
	}
	
	public void javatoXml(Doc expr,String cheminFichier) throws JAXBException, FileNotFoundException{
		ManipXML x = new ManipXML();
		//Traitons soient les donnees, soit les Artefacts
		Doc res = new Doc();
		res.setArtefact(x.creerDocXML(expr.getArtefact()));
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public Artefact contruirMessage(Message msg){
		Artefact newArt= new Artefact();
		
		Address add = new Address();
		   add.setNom("undefined");
		   add.setUrl("undefined");
		   add.setPort(Integer.parseInt(msg.getPortDistant()));
		newArt.setAddress(add);
		
		Data data = new Data();
			Elt e = new Elt();
			e.setName(msg.getNom());
			e.setType(msg.getPortLocal());	
		data.setElt(e);
		...
		for( Object o1 : artefact.getArtefactOrData()){
			...
			try{
				...
				if(artefact.getAddress() != null){
					...
					System.out.println(">>>> Nom "+message.getNom());
					System.out.println(">>>> Local "+message.getPortLocal());
					System.out.println(">>>> Distant "+message.getPortDistant());
				}
				///////////////////////////////////////////////////
	...
	public Artefact creerDocXML(Artefact a){
		Artefact art = new Artefact();
		List l = new ArrayList<Object>();
