...
public class SendReceive {
    ...
    public void sendReceive(Msg msgRecu) throws JAXBException, IOException, JSONException, FileNotFoundException, InterruptedException {
        ...
        while (serviceFoundList == null) {
            ...
            while (operationChoisie.equals("")) {
                ...
                operationChoisie = FileOfFolder.lireValeur(new File(Start.gagconfigFile.getMutexFile()+"toto.txt"));//= br.readLine();
            }
            ...
            System.err.println("operationChoisie  " + operationChoisie);
            System.err.println("schemaExecution " + schemaExecution);
            ...
            System.err.println("serviceFoundList "+serviceFoundList);
        ...
        FileUtils.writeStringToFile(new File(Start.gagconfigFile.getMutexFile()+"instance.txt"), this.instanceFolder);
        ...
        if (serviceFoundList.size() > 1) {
            ...
            for (int i = 0; i < serviceFoundList.size(); i++) {
                System.out.println("schemaExecution = "+schemaExecution);
                System.out.println("ahahhh = "+serviceFoundList.get(i).split("::")[2]);
        ...
        System.out.println("srcPath "+serviceFoundList.get(numChoisi).split("::")[0]);
        System.out.println("destPath "+serviceFoundList.get(numChoisi).split("::")[1]);
        System.out.println("instance "+serviceFoundList.get(numChoisi).split("::")[2]);
        ...
        if (service.getType().equals("simple")) {
            ...
            for (int i = 0; i < service.getUse().size(); i++) {
                ...
                if (service.getUse().get(i).getType().equals("simple")) {
                    ...
                    treatmentCompositeService(config, service, service.getUse().get(i), msgRecu, serviceFound, i);
                }
            }
        }
    ...
    public String displayService(String packageService, String urlFile) throws JAXBException {
        ...
        try {
            //Signaler au frondEnd le service à executer
            FileUtils.writeStringToFile(new File(Start.gagconfigFile.getMutexFile()+"serviceAchoisir.txt"), serviceName.trim());
            //fin signalement
    ...
    public Service defineContexte(Service service, Msg msgRecu, Config config) {
        ...
        for (int l = 0; l < service.getContexte().getItem().size(); l++) {
            if (service.getContexte().getItem().get(l).getContent().equals("")) {
                ...
                while (s.equals("")) {
                    //BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
                    s = FileOfFolder.lireValeur(new File(Start.gagconfigFile.getMutexFile()+"toto.txt"));//= br.readLine();
                }
    ...
    public void treatmentCompositeService(Config config, Service service, Use currentUse, Msg msgRecu, String serviceFound, int i) throws JAXBException, FileNotFoundException, IOException, InterruptedException, JSONException {
...
//signaler au frontend kon travaille
FileUtils.writeStringToFile(new File(Start.gagconfigFile.getMutexFile()+"work.txt"), "work");
...
FileUtils.writeStringToFile(new File(Start.gagconfigFile.getMutexFile()+"work.txt"), "");
        ...
        for (int k = 0; k < currentUse.getInputParameter().getItem().size(); k++) {
            ...
            if (currentUse.getInputParameter().getItem().get(k).getContent() == null) {
                ...
                FileUtils.writeStringToFile(new File(Start.gagconfigFile.getMutexFile()+"besoin.txt"), service.getUse().get(indice).getInputParameter().getItem().get(k).getName());
                
                //Completer l'input
                String input = "";
                while (input.equals("")) {
                    //BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
                    input = FileOfFolder.lireValeur(new File(Start.gagconfigFile.getMutexFile()+"toto.txt"));//= br.readLine();
                }
    ...
    public static void setInstanceFolder(String instanceFolder) {
        SendReceive.instanceFolder = instanceFolder;
    }
